...
double inline dist(ci a, ci b) {
  return abs(a - b);
}

// Theta in radiens
ci inline rotate(ci a, double theta) {
...
double distToLine(ci p, ci a, ci b) {
  ...
  ci ab = a - b;
  double u =
      dot(ap, ab) / (abs(ab) * abs(ab));
  ci c =
      a +
      ab * u;  // This is our closest point
  return abs(p - c);
}
...
double distToLineSegment(ci p, ci a, ci b) {
  ...
  if (u > 1.0)
    return dist(p, b);  // b is closest
  return distToList(
      p, a, b);  // closest is in segment.
}
...
bool collinear(ci p, ci q, ci r) {
  return abs(dir(p, q, r)) < EPS
}
